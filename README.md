...
참고로 C언어를 예로 들면 함수의 인자로 포인터를 넘기는 것을 call by reference라고 알고 있거나, ++i는 변수 내용이 먼저 증가하는 것이고 i++는 나중에 증가하는 것이다 라고 알고 있다면 본인도 잘 못 알고 있는 것입니다. 이 경우 C언어로 일해야하는 상황이라면 위에 있는 자료로 다시 학습하기를 권장합니다. (개인적으로 이런 식으로 잘 못 알고 있는건 학습자의 잘못이 아니라 공부한 서적 혹은 가르쳐준 사람의 잘못이라고 생각합니다.)
